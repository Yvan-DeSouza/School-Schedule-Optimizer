...
def test_student_course_request_ownership(authenticated_client, course, academic_year, student_user, second_student_user, counselor_user, teacher_user):
    second_course = Course.objects.create(name="Physics", grade_level=12, course_code="SPH4U", category="science", capacity_min=10, capacity_max=30)
    client = authenticated_client(student_user)
    own = {"academic_year": academic_year.id, "course": course.id, "request_type": "primary", "is_mandatory": False}
    ...
    spoofed = {"academic_year": academic_year.id, "course": second_course.id, "request_type": "alternate", "student": second_student_user.student_profile.id}
    assert client.post("/api/course-requests/", spoofed, format="json").status_code == 400
    other = CourseRequest.objects.create(student=second_student_user.student_profile, academic_year=academic_year, course=second_course, request_type="primary")
